...
def process(InputDirectory, OutputDirectory, filters, displayFilters, filter3, filter4, weatherFilter):
    ...
    if len(filters) != 0:
        ...
        for file in os.listdir(InputDirectory):
            if file.endswith(".pcap") and file != "TimestampsDuctingAndIcePeriod.csv":
                ...
                if datefilter == True:
                    try:
                        ...
                        continue
                # If a date filter has been inputted, check to see if .pcap file is within specified time-frame
                if (datefilter == True and date >= filter3 and date <= filter4) or datefilter == False:
                    # If a weather filter has been inputted, check to see if .pcap file met condition and is in dictionary
                    if (weatherFilt is True and file in weatherFilter) or weatherFilt is False:
                        ...
                        with open(OutputDirectory + "\\" + filename2, "w") as outfile:
                            command = []
                            commandBegin = ["tshark.exe", "-r", os.path.join(InputDirectory, file), "-o", "tcp.calculate_timestamps:true"]
                            ...
                            for fltr in filters:
                                command.append("-e")
                                command.append(fltr)
                            commandEnd = ["-T", "fields", "-E", "separator=,", "-E", "occurrence=f", "-N", "dm"]
...
def process_flows(proto, InputDirectory, OutputDirectory, filter3, filter4, weatherFilter):
    datefilter, weatherFilt = False, False
    if filter3 != "" and filter4 != "": datefilter = True
    if len(weatherFilter) != 0: weatherFilt = True
        ...
        if file.endswith(".pcap") and file != "TimestampsDuctingAndIcePeriod.csv":
            ...
            if (datefilter == True and date >= filter3 and date <= filter4) or datefilter == False:
                ...
                if (weatherFilt is True and file in weatherFilter) or weatherFilt is False:
                    filename2 = os.path.splitext(file)[0] + '.csv'
                    with open(OutputDirectory + "\\" + filename2, "w") as outfile:
                        if proto == 1: command = ["tshark.exe", "-r", os.path.join(InputDirectory, file), "-N", "dm", "-q", "-z", "conv,tcp"]
                        else: command = ["tshark.exe", "-r", os.path.join(InputDirectory, file), "-N", "dm", "-q", "-z", "conv,udp"]
                        try:
                ...
                for row in csv_reader:
                    ...
                    if fieldList[0] != "================================================================================":
                        if i > 5:
                            ...
                            fieldList2 = [sourceIP, sourcePort, destIP, destPort, fieldList[9], totPktsSize, fieldList[6],
                                          sourcePktsSize, fieldList[3], dstPktsSize, fieldList[12], fieldList[13]]
            ...
            if file.endswith(end):
                ...
                with open(OutputDirectory + "\\" + filename2, "w") as outfile:
                    try:
                        ...
                        command = ["tshark.exe", "-r", os.path.join(InputDirectory, filename1), "-Y", "tcp.flags.fin==1", "-e", "ip.src",
                                   "-e", "tcp.srcport", "-e", "ip.dst", "-e", "tcp.dstport", "-e", "tcp.stream", "-T", "fields", "-E",
                                   "separator=,", "-E", "occurrence=f", "-N", "dm"]
                        subprocess.run(command, stdout=outfile, check=True)
                    except Exception as err:
                        print(err)
