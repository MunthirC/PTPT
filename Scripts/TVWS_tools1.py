...
def calc(func, filt, filters, codeOrService, directory, proto, filter3, filter4, weatherFilter):
    global count
    global total
    global average
                    ...
                    if func == "count":
                        ...
                        if filt in filters and filt != "http.response.code":
                            try:
                                ...
                                proc = subprocess.Popen("awk -F \",\" \'BEGIN{count=0}{if($"+indx+"!=\"\")count=count+1}END{print count}\' "
                        ...
                        if filt in filters:
                            try:
                                ...
                                #subprocess.run(command)
                                proc = subprocess.Popen("awk -F \",\" \'BEGIN{total=0}{if($"+indx+"!=\"\")total=total+$"+indx+"}END{print total}\' "
                                                        + os.path.join(directory, file), shell=True, stdout=subprocess.PIPE)
                        ...
                        if filt in filters:
                            try:
                                ...
                                #subprocess.run(command)
                                proc = subprocess.Popen("awk -F \",\" \'BEGIN{total=0;count=0}{if($"+indx+"!=\"\")total=total+$"+
