...
def tableGraph(outdir, indir, filters, fil, index, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2):
    err = False

    try:
        if fil not in ["genStats", "table1", "genTCPFlowStats", "table2", "udTCPFlowStats", "table3", "httpCodes", "table4",
                       "sfTCPFlowStats", "table6"]:
            func, fiel, data = customTable(indir, filters, fil, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2)
            if outdir is not None: path = outdir + "/table_" + str(index) + ".png"
        else:
            if fil == "genStats" or fil == "table1":
                func, fiel, data = table1(indir, filters, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2)
            elif fil == "genTCPFlowStats" or fil == "table2":
                func, fiel, data = table2(indir, filters, avgCalcs, totalCalcs, dateFilter1, dateFilter2)
            elif fil == "udTCPFlowStats" or fil == "table3":
                func, fiel, data = table3(indir, filters, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2)
            elif fil == "httpCodes" or fil == "table4": func, fiel, data = table4(indir, filters, dateFilter1, dateFilter2)
            else: func, fiel, data = table6(indir, filters, countCalcs, dateFilter1, dateFilter2)
            if outdir is not None: path = outdir + "/table_" + fil + ".png"
...
def customTable(indir, filters, fil, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2):
    fields = ""
    tableFields = []
    table_data = []
    ...
    if len(tableFields) != 0:
        ...
        for field in tableFields:
            ...
            # Else, calculate data point using 'calc' function via 'TVWS_tools1'
                ...
                if len(field) == 2:
                    table_data.append(TVWS_tools1.calc(field[0], field[1], filters, "", indir, 1, dateFilter1, dateFilter2))
                else:
                    table_data.append(TVWS_tools1.calc(field[0], field[1], filters, field[2], indir, 1, dateFilter1, dateFilter2))
            func.append(field[0])
...
def table1(indir, filters, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2):
    table_data = []

    if "frame.len" not in totalCalcs:
        table_data.append(TVWS_tools1.calc("total", "frame.len", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(totalCalcs["frame.len"])
    if "frame.len" not in countCalcs:
        table_data.append(TVWS_tools1.calc("count", "frame.len", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(countCalcs["frame.len"])
    if "cpkt" not in countCalcs:
        table_data.append(TVWS_tools1.calc("count", "cpkt", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(countCalcs["cpkt"])
    if "tcp.analysis.ack_rtt" not in avgCalcs:
        table_data.append(TVWS_tools1.calc("avg", "tcp.analysis.ack_rtt", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(avgCalcs["tcp.analysis.ack_rtt"])
    if "tcp.analysis.retransmission" not in countCalcs:
        table_data.append(TVWS_tools1.calc("count", "tcp.analysis.retransmission", filters, "", indir, 1, dateFilter1, dateFilter2))
...
def table2(indir, filters, avgCalcs, totalCalcs, dateFilter1, dateFilter2):
    table_data = []

    if "flwsiz_tcpflows" not in totalCalcs:
        table_data.append(TVWS_tools1.calc("total", "flwsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(totalCalcs["flwsiz_tcpflows"])
    if "flwsiz_tcpflows" not in avgCalcs:
        table_data.append(TVWS_tools1.calc("avg", "flwsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
...
def table3(indir, filters, avgCalcs, totalCalcs, countCalcs, dateFilter1, dateFilter2):
    table_data = []

    if "flw_tcpflows" not in countCalcs:
        table_data.append(TVWS_tools1.calc("count", "flw_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(countCalcs["flw_tcpflows"])
    if "upflwsiz_tcpflows" not in totalCalcs:
        table_data.append(TVWS_tools1.calc("total", "upflwsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(totalCalcs["upflwsiz_tcpflows"])
    if "dwflwsiz_tcpflows" not in totalCalcs:
        table_data.append(TVWS_tools1.calc("total", "dwflwsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(totalCalcs["dwflwsiz_tcpflows"])
    if "upflwsiz_tcpflows" not in avgCalcs:
        table_data.append(TVWS_tools1.calc("avg", "upflwsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(avgCalcs["upflwsiz_tcpflows"])
    if "dwflwsiz_tcpflows" not in avgCalcs:
        table_data.append(TVWS_tools1.calc("avg", "dwflwsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(avgCalcs["dwflwsiz_tcpflows"])
    if "uppktsiz_tcpflows" not in avgCalcs:
        table_data.append(TVWS_tools1.calc("avg", "uppktsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(avgCalcs["uppktsiz_tcpflows"])
    if "dwpktsiz_tcpflows" not in avgCalcs:
        table_data.append(TVWS_tools1.calc("avg", "dwpktsiz_tcpflows", filters, "", indir, 1, dateFilter1, dateFilter2))
...
def table4(indir, filters, dateFilter1, dateFilter2):
    table_data = []

    table_data.append(TVWS_tools1.calc("count", "http.response.code", filters, "200", indir, 1, dateFilter1, dateFilter2))
    table_data.append(TVWS_tools1.calc("count", "http.response.code", filters, "400", indir, 1, dateFilter1, dateFilter2))
    table_data.append(TVWS_tools1.calc("count", "http.response.code", filters, "408", indir, 1, dateFilter1, dateFilter2))
...
def table6(indir, filters, countCalcs, dateFilter1, dateFilter2):
    table_data = []

    if "successfulTCPFlows" not in countCalcs:
        table_data.append(TVWS_tools1.calc("count", "successfulTCPFlows", filters, "", indir, 1, dateFilter1, dateFilter2))
    else: table_data.append(countCalcs["successfulTCPFlows"])
    if "failedTCPFlows" not in countCalcs:
        table_data.append(TVWS_tools1.calc("count", "failedTCPFlows", filters, "", indir, 1, dateFilter1, dateFilter2))
